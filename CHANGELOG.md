...
### Changed
- `run_command_grep`: a zero-match response no longer returns a bare `(žiadna zhoda)` — it now states what was scanned: `(žiadna zhoda — 0 z N riadkov výstupu, exit code 0)` on the normal path, and `(žiadna zhoda — výstup prázdny, exit code M)` / `exit code neznámy` (signal/timeout) in the catch block. `Nájdených N zhôd: …` is unchanged. Motivation: the same text was returned for "output scanned, nothing matched" and "command produced nothing", which was misread as a lost command (feedback 2026-09-16 — the only actionable part of a report that was otherwise not reproducible). New helper `formatZeroMatch()` in `src/format.ts` + 3 tests (total 354).
...
- **POSIX: the command-target safety checks were silently Windows-only.** `isFlag()` tested `^\/[a-z]` unanchored, and on POSIX every absolute path starts with `/<letter>` — so an absolute POSIX path was read as a cmd flag and `findOutOfRootWriteTargets` never saw the target of `mkdir /tmp/x`, `mkdir -p /tmp/x` or `copy a.txt /tmp/x` (worst case the `copy`/`move` branch fell back to `a.txt`). `QUOTED_PATH` (cross-root reads, active with `MCP_BLOCK_CROSS_ROOT_READS=1`) matched `C:\`, `../` and `~/` only, so quoted absolute POSIX reads were invisible. Both are fixed: `isFlag()` now accepts a `/x` flag only as a 1–2 letter token, and `QUOTED_PATH` gained a POSIX absolute branch (a quoted token is a path, never a flag). The 5 previously skipped assertions run again, plus 3 new platform-neutral regression tests (`mkdir`/`copy` at the filesystem root, quoted absolute read) that fail on the old code even on Windows.

### Known Limitations (documented)
- Residual: a 1–2 letter directory directly under the filesystem root (`/a`) is still read as a cmd flag by `isFlag()` — use a longer name or the unquoted full path form for such targets.
