...
* **Styling:** N/A
* **Testing:** Vitest — `npm test` (147 tests across `output`, `safety`, `format`, `redirect` suites, run only on `src/__tests__` — `build/` is excluded from the test pattern).
* **File Structure:**
  * `src/` — TypeScript sources (`index.ts`, `safety.ts`, `output.ts`, `format.ts`, `redirect.ts`, `__tests__/`)
...
| `npm test` | Run Vitest unit tests (147 tests, `src/__tests__` only) |
