...
* **Testing:** Vitest — `npm test` (348 tests across `output`, `safety`, `format`, `redirect`, `symbols`, `split`, `batch`, `skeleton`, `verify`, `feedback`, `tool-registry`, `handlers`, `tool-integration` suites, run only on `src/__tests__` — `build/` is excluded from the test pattern). Keep this count, the README badge and the README version line in sync on every release (single source: the actual `npm test` output).
* **File Structure:**
...
| `npm test` | Run Vitest unit tests (348 tests, `src/__tests__` only) |
